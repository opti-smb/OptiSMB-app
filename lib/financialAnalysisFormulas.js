 ...
 * Single source of truth for those formulas in the app. (See also `computeBenchmarkAnalysis.js` for benchmark payloads.)
 *
...
/** Fees at benchmark rate (%, e.g. 2.1): Revenue × Rate / 100 */
export function feesAtBenchmarkRate(totalGrossVolume, benchmarkRatePct) {
  const gv = Number(totalGrossVolume);
  const b = Number(benchmarkRatePct);
  if (!(gv > 0)) return 0;
  return roundMoney2((gv * b) / 100);
}

export function estimatedOverpaymentVsBenchmark(totalFeesCharged, feesAtBenchmark) {
  return roundMoney2(Number(totalFeesCharged) - Number(feesAtBenchmark));
}

export function rateGapPercentagePoints(yourEffectiveRatePct, benchmarkRatePct) {
  if (yourEffectiveRatePct == null || Number.isNaN(Number(yourEffectiveRatePct))) return null;
  return roundRate4(Number(yourEffectiveRatePct) - Number(benchmarkRatePct));
}
