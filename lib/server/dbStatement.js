...
/** Snapshot POST body so we never persist a shared object the client may mutate between uploads. */
...
export function dbStatementToClient(row) {
  ...
  let snap = {};
  if (pr?.parsedSnapshot && typeof pr.parsedSnapshot === 'object' && !Array.isArray(pr.parsedSnapshot)) {
    const deep = cloneJsonSafe(pr.parsedSnapshot);
    if (deep && typeof deep === 'object') snap = deep;
  }
  ...
  return {
    id: row.statementId,
    uploadedAt: row.uploadedAt instanceof Date ? row.uploadedAt.toISOString() : undefined,
...
/**
 * Prefer the upload payload filename; if it disagrees with parser metadata, trust the parser
 * (avoids stale React `fileName` while `parsedData` is already the new parse).
 * @param {Record<string, unknown>} body
 * @param {Record<string, unknown>} pd
 */
function parserSuggestedFilename(pd) {
...
function resolveStatementFileName(body, pd) {
  const fromBody = typeof body?.fileName === 'string' ? body.fileName.trim() : '';
  const fromParser = parserSuggestedFilename(pd);
  if (fromParser && fromBody && fileNorm(fromParser) !== fileNorm(fromBody)) {
    return fromParser;
  }
...
export async function createStatementForUser(userId, body) {
  const b = cloneJsonSafe(body);
  if (!b || typeof b !== 'object') {
    throw new Error('body_required');
  }
  const rawPd = b.parsedData && typeof b.parsedData === 'object' ? b.parsedData : {};
  ...
  const resolvedFileName = resolveStatementFileName(b, pd).slice(0, 2048);

  const clientBlock = {
    fileName: resolvedFileName,
    fileType: b.fileType,
    statementCategory: b.statementCategory,
    uploadDate: b.uploadDate,
    period: b.period,
    source: b.source,
    parseMethod: b.parseMethod,
    rateConfidence: b.rateConfidence,
    dataAsOf: b.dataAsOf,
    uploadKindDescription: b.uploadKindDescription,
    parseFailureReason: b.parseFailureReason,
    parseFailureMessage: b.parseFailureMessage,
    extractionRatio: b.extractionRatio,
    discrepancies: b.discrepancies,
    benchmarks: b.benchmarks,
    rateTrend: b.rateTrend,
  };
  ...
  const fromIso = b.billingPeriodFromIso || (pd.billing_period?.from ? String(pd.billing_period.from).slice(0, 10) : null);
  const toIso = b.billingPeriodToIso || (pd.billing_period?.to ? String(pd.billing_period.to).slice(0, 10) : null);

  const size = Number(b.fileSizeBytes);
  const fileSizeBytes = Number.isFinite(size) && size >= 0 ? BigInt(Math.floor(size)) : 0n;

  const sha =
    b.fileSha256 && /^[a-f0-9]{64}$/i.test(String(b.fileSha256))
      ? String(b.fileSha256).toLowerCase().slice(0, 64)
      : null;

  const fromD = billingDate(fromIso);
  const toD = billingDate(toIso);
  const acqForDb = normAcquirerStr(b.acquirer) || normAcquirerStr(pd.acquirer_name);

  if (sha) {
    const existingBySha = await prisma.statement.findFirst({
      where: { userId, fileSha256: sha },
      orderBy: { statementId: 'desc' },
      include: { parsedData: true },
    });
    if (existingBySha) {
      return { row: existingBySha, duplicate: true };
    }
  }

  if (acqForDb && fromD && toD) {
    const existingBySlot = await prisma.statement.findFirst({
      where: {
        userId,
        acquirerName: acqForDb,
        billingPeriodFrom: fromD,
        billingPeriodTo: toD,
      },
      orderBy: { statementId: 'desc' },
      include: { parsedData: true },
    ...
    if (existingBySlot) {
      return { row: existingBySlot, duplicate: true };
    }
  }
